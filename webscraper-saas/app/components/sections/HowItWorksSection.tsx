...
const steps = [
  ...
  { n: "02", icon: ScanSearch, title: "Intelligent Crawl",       body: "SourcePull BFS engine visits every discoverable page, following internal links and collecting assets.", snippet: "23 pages — 8.2 MB", tag: "CRAWL" },
...
// Extract each step into its own component so hooks are called at the top level (not inside .map())
function Step({ s, i, isLast }: { s: typeof steps[0]; i: number; isLast: boolean }) {
  const Icon = s.icon;
  const sRef = useRef(null);
  const sInView = useInView(sRef, { once: true, margin: "-60px" });

  return (
    <div ref={sRef} className="relative flex gap-7">
      {!isLast && (
