...
/* createZipBuffer — streams directly into memory, no file write needed.
   This is the key fix for Vercel: /tmp doesn't persist across function calls,
   so we return the ZIP as base64 in the JSON response and let the client
   decode + download it entirely in the browser. */
async function createZipBuffer(sourceDir: string): Promise<Buffer> {
  ...
  return new Promise((resolve, reject) => {
    ...
    passthrough.on("data", (chunk: Buffer) => chunks.push(Buffer.isBuffer(chunk) ? chunk : Buffer.from(chunk)));
